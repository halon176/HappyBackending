...
class EventResponse(CustomBase):
    ...
    date: datetime
    location: str = Field(
        ..., min_length=3, max_length=50, examples=["Piazza del paese"]
    )
...
class EventUpdate(CustomBase):
    name: Optional[str] = Field(
        None, min_length=3, max_length=50, examples=["Sagra della patata"]
    )
    date: Optional[datetime] = Field(None)
    location: Optional[str] = Field(
        None, min_length=3, max_length=50, examples=["Piazza del paese"]
    )
